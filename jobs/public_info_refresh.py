...
def users_update():
    ...
    for user_doc in user_cursor:
        user_update(user_doc['CharacterID'])
...
def corps_update():
    ...
    for corp_doc in corp_cursor:
        corp_update(corp_doc['corporation_id'])
...
def corp_update(corporation_id, data_to_update={}):
    ...
    corporation_filter = {'corporation_id': corporation_id}
    update = {"$set": data_to_update}
...
def alliances_update():
    ...
    for alliance_doc in alliance_cursor:
        alliance_update(alliance_doc['alliance_id'])
